...
function main(order, back, config_path = defaultConfig.path) {
  ...
  if (config_path == defaultConfig.path && defaultConfig.size === 0) console.error('Error: the default connection is unset!')
    ...
    order = (back ? '' : isRoot() ? '' : 'sudo ') + '/usr/bin/python ' + config.get('ssr-path') + '/shadowsocks/local.py --fast-open -c ' + config_path + ' -d ' + order
